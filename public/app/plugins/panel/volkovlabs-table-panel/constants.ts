...
type SelectorFactory = {
  ...
  apply: (...args: unknown[]) => Record<string, string>;
};

const createSelector = (
  selectorOrFactory: string | ((...args: unknown[]) => string),
  attr = 'aria-label'
): SelectorFactory => {
  const getSelector = (...args: unknown[]) =>
    typeof selectorOrFactory === 'function' ? selectorOrFactory(...args) : selectorOrFactory;
  ...
  return {
    ...
    apply: (...args: unknown[]) => {
      ...
      const [key, ...valueParts] = selector.split(' ');
...
export const TEST_IDS = {
  panel: {
    ...
    errorContainer: createSelector('data-testid panel error-container'),
    errorAlertElement: createSelector('data-testid panel error-alert-element'),
  },
  tablesEditor: {
  ...
  editableColumnEditor: {
    ...
    fieldLocalTime: createSelector('data-testid column-editor field-local-time'),
    fieldFileAcceptTypes: createSelector('data-testid column-editor field-file-accept-types'),
  },
  dateEditor: {
