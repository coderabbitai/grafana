...
import { CellAggregation, CellType, ColumnMeta } from 'app/plugins/panel/volkovlabs-table-panel/types';
import { createColumnConfig, createField } from 'app/plugins/panel/volkovlabs-table-panel/utils';
...
describe('AggregatedCellRenderer', () => {
  ...
  it('Should add inherit color if background not specified', async () => {
    ...
    expect(selectors.root()).toBeInTheDocument();
    expect(selectors.root()).toHaveTextContent('123%');
    expect(getComputedStyle(selectors.root()).color).toEqual('inherit');
  });
